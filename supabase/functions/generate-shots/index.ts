        ...
        const aiResponse = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              ...
              {
                role: "user",
                content: `Generate ${labels.length} image prompt(s) for a ${category} product. Style: ${preset}. Shot types: ${labels.join(", ")}. ${shotType === "model_shot" && modelConfig ? `Model: ${modelConfig.gender || ""} ${modelConfig.ethnicity || ""} ${modelConfig.bodyType || ""}. Background: ${modelConfig.background || "studio"}.` : "Product showcase, no model."} ${additionalContext ? `Additional direction: ${additionalContext}` : ""}`,
              },
            ],
            tools: [
              {
