...
OUTPUT: Generate exactly ONE single photograph. Do NOT create a collage, grid, mosaic, contact sheet, or multiple images combined. ONE image, ONE model, ONE pose, ONE composition.`;
    ...
    async function generateSingleShot(label: string, prompt: string): Promise<any | null> {
      ...
      const callAI = async (overridePrompt?: string) => {
        ...
        const vertexUrl = `https://us-central1-aiplatform.googleapis.com/v1/projects/${gcpProjectId}/locations/us-central1/publishers/google/models/gemini-3.1-flash:generateContent`;
      ...
      // Skip in-function upscale to avoid compute limits
      // Upscaling 6 images inside one edge function exceeds resource limits
      // TODO: implement separate upscale endpoint or background job
